...
function extrairInfo (textoPdf: string ,textoReferencia: string, linhaReferente: number, indice: number): string {
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  if (!nr_cliente || !mes_referencia || !energia_ele_kwh || !energia_ele_valor || !energia_scee_kwh || !energia_scee_valor || !energia_compensada_kwh || !energia_compensada_valor || !contrib_ilum_valor) {
    return Response.json({ msg: 'Erro ao extrair informações do PDF!' }, { status: 400 });
  }
